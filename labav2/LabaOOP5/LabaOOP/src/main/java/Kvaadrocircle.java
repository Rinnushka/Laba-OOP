...
public class Kvaadrocircle implements IVehicle {
    private String marka;
    private ArrayList<Model> model;
    int size;
    public Kvaadrocircle(String marka, int count ){
        model = new ArrayList<>();
        this.marka = marka;
        size = count;
    ...
    public Object clone(){
        try{
            Kvaadrocircle kv = (Kvaadrocircle) super.clone();
            kv.model = (ArrayList<Model>) model.clone();
            kv.model.clear();
    ...
    public boolean equals(Object obj){
        ...
        if(obj instanceof Kvaadrocircle){
            Kvaadrocircle kvadrocircle = (Kvaadrocircle) obj;//получаем доступ к полям класса
    ...
    public String toString(){
        ...
        buff.append("Тип - Квадроцикл").append("\n").append("Marka: ").append(marka).append("\n");
        for(int i =0; i<model.size();i++){
    ...
    public void addModel(String nameNewModel, double priceNewModel)throws DuplicateModelNameException {
        ...
        if(i==model.size()){
            model.add(new Model(nameNewModel,priceNewModel));
        }
    }
